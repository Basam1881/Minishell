...
/*   Updated: 2021/12/06 03:21:15 by bnaji            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
*/
static void	check_arg_helper1(char *s, int *x, int *single_qoute, int *double_qoute)
{
	if (s[*x] == '\'' || s[*x] == '"')
	{
		if (s[*x] == '\'' || s[*x] == '"')
...
*/
static void	check_arg_helper2(char **str, int *i, int *j, int flag)
{
	if (flag)
	{
		if (flag == 2)
...
static void	check_arg(char **str, char *s, int flag, int *x, int *i)
{
	...
	if (!flag)
		(*i)++;
	while (s[*x] && (s[*x] != ' ' || (s[*x] == ' ' && (single_qoute || double_qoute))))
	{
		check_arg_helper1(s, x, &single_qoute, &double_qoute);
		if (!((s[*x] == '\'' && single_qoute) || (s[*x] == '"' && double_qoute)))
		{
			if (flag == 2)
				str[*i][j] = s[*x];
	...
	check_arg_helper2(str, i , &j, flag);
}
...
/*
	...
	flag 2 will copy the proper value from the string (cmdline) to the arguments you have
*/
